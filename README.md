...
2. Wait for a while while drinking coffee (optional), as it takes time to copy the files. 3.   
...
Download vosk-model-ja-0.xx.zip (where xx is the version number) from https://alphacephei.com/vosk/models.   
   vosk-model-en-0.xx.zip   
   Latest version as of July 2023 https://alphacephei.com/vosk/models/vosk-model-ja-0.22.zip   
   The download takes quite a while, so have a cup of coffee (optional) and wait a while. 3.   
Unzip the downloaded zip file (again, wait a while).   
  Rename "vosk-model-ja-0.xx" to "model-ja" in the extracted folder. 4.   
Create a folder called vosk-model in ontama and move all the model-ja from 3 into vosk-model.   
  Again, wait for a while. 5.   
...
A black screen appears, wait a moment and a menu will appear. 3.   
Select "File(wav, mp3, mp4)" if you want to recognise the sound in an audio or video file. 3-1.   
  3-1. A screen for selecting a file appears, select an audio or video file and click 'Open'.   
  3-2. The black screen in 3-2 shows the progress of the various operations. 3-3.   
  3-3. "***.docx" and "***_plani.txt" files are created in the same folder as the file selected in 3-1 (*** is the same name as the input file). Delete if unnecessary). 4.   
4. Select 'Microphone' if you want to recognise the sound input from the microphone of the PC.   
  4-1. Wait for a while as various displays appear on the black screen. 4-2.   
  ...
  Press Ctrl+C to STOP 4-4.   
  The files "yyyy_mm_dd_hh_mm_ss.docx" (year_month_day_hour_minute_second) and "yyyy_mm_dd_hh_mm_ss_plain.txt" are created in the same folder as ontama.exe.   
